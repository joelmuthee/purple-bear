...
(async function () {
  ...
  const NEW_DAYS = 7;
  const LOW_STOCK = 3;
  const WISHLIST_KEY = 'purplebear_wishlist';
  let items = [];
  let settings = {};
  let suspended = false;
  ...
  const ANALYTICS_KEY = 'purplebear_analytics';
  ...
  // Build the inner size-chip buttons for a list of sizes.
  ...
  function enquireBody(item, soldOut, selectedSize, selectedColor) {
    ...
    return soldOut
      ? `Hi Purple Bear! I saw *${item.name}* is sold out. Will it be back in stock? I'd love to reserve one.`
      : `Hi Purple Bear! I'd like to enquire about *${item.name}*${colorPart}${sizePart}${pricePart} from your catalog.`;
  }
  ...
  const IG_SVG = `<svg class="ig-icon" viewBox="0 0 24 24" width="15" height="15" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M21 12a9 9 0 1 1-3-6.7"></path><polyline points="21 3 21 9 15 9"></polyline></svg>`;
  ...
  function render() {
    ...
    gallery.innerHTML = visible.map(item => {
      ...
      const colorsHtml = (!soldOut && cols.length)
        ...
        : '';
      // Colour-stock items: sizes appear after a colour is picked (start empty,
      // marked data-color-driven so the colour handler fills it). Others: aggregate sizes.
  ...
  gallery.addEventListener('click', async e => {
    ...
    if (enquire) {
      ...
      if (!soldOut && itemHasColorStock(item)) {
        ...
        // Flat-stock items: require size selection when multiple sizes exist.
  ...
  document.getElementById('wishlistEnquireAll')?.addEventListener('click', e => {
    ...
    const phone = settings.whatsappNumber || '254725672399';
    // Each item carries its /p/<id> share link so Purple Bear can open the exact item.
  ...
  function showSuspended() {
    ...
    const css = ('@keyframes rkSusFade{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}'
      + '#suspendedOverlay{position:fixed;inset:0;z-index:99999;background:radial-gradient(ellipse at top,#3d2817 0%,#2a1c0f 65%);color:#e8dcc4;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;padding:40px 24px;font-family:Inter,system-ui,-apple-system,sans-serif;animation:rkSusFade 0.65s ease both;}'
      + '#suspendedOverlay .rk-logo{width:140px;height:140px;border-radius:50%;object-fit:cover;background:#fff;border:2px solid #b8956a;box-shadow:0 0 36px rgba(184,149,106,0.4),inset 0 0 0 1px rgba(255,255,255,0.04);margin-bottom:26px;}'
      + '#suspendedOverlay .rk-name{font-family:\'Cormorant Garamond\',Georgia,serif;font-size:34px;color:#d4c0a0;letter-spacing:2.5px;font-weight:500;line-height:1;margin-bottom:8px;}'
      + '#suspendedOverlay .rk-tag{font-size:12px;color:#b8956a;letter-spacing:2px;text-transform:uppercase;margin-bottom:30px;opacity:0.9;}'
      + '#suspendedOverlay .rk-rule{width:54px;height:1px;background:linear-gradient(90deg,transparent,#b8956a,transparent);margin-bottom:30px;}'
      + '#suspendedOverlay .rk-head{font-family:\'Cormorant Garamond\',Georgia,serif;font-weight:500;font-size:clamp(30px,5vw,44px);margin:0 0 16px;color:#e8dcc4;line-height:1.15;}'
      + '#suspendedOverlay .rk-body{font-size:16px;max-width:460px;line-height:1.65;opacity:0.82;margin:0 0 14px;}'
      + '#suspendedOverlay .rk-offer{font-size:16px;max-width:460px;line-height:1.6;margin:0 0 30px;color:#d4c0a0;}'
      + '#suspendedOverlay .rk-offer b{color:#fff;font-weight:700;}'
      + '#suspendedOverlay .rk-ig{display:inline-flex;align-items:center;gap:10px;background:#b8956a;color:#2a1c0f;padding:14px 30px;border-radius:999px;text-decoration:none;font-weight:600;font-size:15px;letter-spacing:0.3px;box-shadow:0 6px 24px rgba(184,149,106,0.3);transition:transform 0.2s ease,box-shadow 0.2s ease,background 0.2s ease;}'
      + '#suspendedOverlay .rk-ig:hover{background:#d4c0a0;transform:translateY(-1px);box-shadow:0 8px 28px rgba(184,149,106,0.42);}'
      + '@media (max-width:480px){#suspendedOverlay .rk-logo{width:118px;height:118px;margin-bottom:22px;}#suspendedOverlay .rk-name{font-size:28px;letter-spacing:2px;}#suspendedOverlay .rk-tag{font-size:11px;margin-bottom:24px;}}'
    );
