// Purple Bear API Worker — kids' shoes
...
// Purple Bear sells KIDS' SHOES. Order matters:
// specific models before generic brand fallbacks; brands before generic types.
// Purple Bear sells KIDS' SHOES. Brand/style keyword → (display name, category).
// Specific models/brands first, generic style words last. Kenya context: Bata +
// Toughees own the school-shoe market; Crocs are their own category.
const SHOE_BRANDS = [
  // Sneakers / trainers
  ...
  ["light up",         null,               "Sneakers"],   // light-up kids sneakers
  ["lightup",          null,               "Sneakers"],
  // School shoes — Bata/Toughees/Clarks dominate the KE school market
  ["toughees",         "Toughees",         "School Shoes"],
  ["bata",             "Bata",             "School Shoes"],
  ["clarks",           "Clarks",           "School Shoes"],
  // Crocs / clogs (own category)
  ["crocs",            "Crocs",            "Crocs"],
  [/\bcroc\b/,         "Crocs",            "Crocs"],
  ["clog",             null,               "Crocs"],
  // Boots
  ["timberland",       "Timberland",       "Boots"],
  ["chelsea boot",     null,               "Boots"],
  // Baby / first-walker shoes
  ["first walker",     null,               "Baby Shoes"],
  ["first step",       null,               "Baby Shoes"],
  ["pre walker",       null,               "Baby Shoes"],
  ["prewalker",        null,               "Baby Shoes"],
  ["bootie",           null,               "Baby Shoes"],
  ["booties",          null,               "Baby Shoes"],
  [/\bbaby\b/,         null,               "Baby Shoes"],
  [/\binfant\b/,       null,               "Baby Shoes"],
  [/\bnewborn\b/,      null,               "Baby Shoes"],
  // Generic style fallbacks (no brand) → kids categories
  ["school shoe",      null,               "School Shoes"],
  ["sneaker",          null,               "Sneakers"],
  ["trainer",          null,               "Sneakers"],
  ["canvas",           null,               "Sneakers"],
  ["rubber shoe",      null,               "Sneakers"],
  ["gladiator",        null,               "Sandals"],
  ["open shoe",        null,               "Sandals"],
  [/\bsandals?\b/,     null,               "Sandals"],
  [/\bboots?\b/,       null,               "Boots"],
  ["slides",           null,               "Slides"],
  [/\bslide\b/,        null,               "Slides"],
  ["slipper",          null,               "Slides"],
  ["flip flop",        null,               "Slides"],
  ["loafer",           null,               "Loafers"],
  ["moccasin",         null,               "Loafers"],
  ["mary jane",        null,               "Loafers"],
  ["doll shoe",        null,               "Loafers"],
  ["ballet",           null,               "Loafers"],
  // Generic "shoes" last — kids default is Sneakers (most common item)
  [/\bshoes?\b/,       null,               "Sneakers"],
];

function deriveBrand(caption) {
  let text = (caption || "").toLowerCase().trim();
  // NOTE: no leading-"username " strip here — feed-API captions have no handle
  // prefix, so stripping the first word would eat the real product word.
  const padded = " " + text + " ";
...
const DEFAULT_DESC = "Quality kids' shoes, hand-picked. Photographed exactly as it is. Pick a size below to enquire.";
...
function parseCaptionForBag(caption) {
  ...
  // --- Kids EU shoe sizes (~16-39) ---
  // Captions: "size 25", "EU 25", "#27", "sizes 22-27", "23/24/25", "20 to 30".
  // Strip clearly-non-size numbers first (%, ages, times) to cut false positives.
  const sizeText = " " + lower
    .replace(/\d+\s*%/g, " ")
    .replace(/\d+\s*(?:years?|yrs?|months?|mo|hrs?|hours?|days?)\b/g, " ")
    .replace(/[,/|·]+/g, " ")
    .replace(/\s+/g, " ") + " ";
  const addSize = (n) => { if (n >= 16 && n <= 39) stock[String(n)] = 1; };
  let m;
  // Ranges first: "22-27", "22 to 27", "22 – 27" → every size in the span.
  const consumed = [];
  const rangeRe = /(?<![0-9])(\d{2})\s*(?:-|–|to)\s*(\d{2})(?![0-9])/g;
  while ((m = rangeRe.exec(sizeText)) !== null) {
    const a = parseInt(m[1], 10), b = parseInt(m[2], 10);
    if (a >= 16 && b <= 39 && b >= a && b - a <= 24) {
      for (let n = a; n <= b; n++) addSize(n);
      consumed.push([m.index, m.index + m[0].length]);
    }
  }
  // Standalone two-digit sizes not already inside a parsed range.
  const numRe = /(?<![0-9])(\d{2})(?![0-9])/g;
  while ((m = numRe.exec(sizeText)) !== null) {
    if (consumed.some(([s, e]) => m.index >= s && m.index < e)) continue;
    addSize(parseInt(m[1], 10));
  }
...
function looksLikeProduct(caption) {
  ...
  // Size signal (kids EU shoe size: "size 25", "eu 25", "#27", or a range)
  if (/\b(?:size|sizes|eu)\s*[:\-]?\s*\d{2}\b/i.test(lower)) return true;
  if (/#\s*\d{2}\b/.test(lower)) return true;
  if (/(?<![0-9])\d{2}\s*(?:-|–|to)\s*\d{2}(?![0-9])/.test(lower)) return true;
...
async function classifyPostWithVision(env, caption, imageUrl) {
  ...
  try {
    ...
    const prompt = `You sort Instagram posts from Purple Bear — a Nairobi shop selling KIDS' SHOES (babies, toddlers, children). You're given ONE photo + ONE caption. Decide:
...
3. What category? Pick EXACTLY one from this list — never invent another:
   Sneakers, Sandals, School Shoes, Boots, Crocs, Slides, Loafers, Baby Shoes

Category guide (look carefully — this is the hardest call):
- Sneakers: casual athletic / lifestyle shoes — trainers, canvas, Air Force, Jordan, Adidas, Puma, light-up shoes, rubber shoes.
- Sandals: open shoes with straps, gladiators, summer sandals.
- School Shoes: plain black/brown formal closed school shoes (Bata, Toughees, Clarks).
- Boots: ankle-high or taller — chelsea boots, winter boots, Timberland-style.
- Crocs: rubber clogs with holes (Crocs and lookalikes).
- Slides: slip-on slides, slippers, flip-flops, pool sliders.
- Loafers: slip-on dress/formal shoes, moccasins, mary-janes, ballet flats, girls' doll shoes.
- Baby Shoes: infant / first-walker / pre-walker shoes, soft booties, crib shoes for babies.

This shop ONLY sells kids' shoes. If the photo shows clothing, a bag, an adult-only shoe, or anything that isn't children's footwear, set is_product=false. (Sandals and slides ARE valid here — they're kids' footwear.)
...
async function classifyPostWithAi(env, caption) {
  ...
  const trimmed = caption.replace(/\s+/g, " ").slice(0, 400);
  const prompt = `You sort Instagram posts from Purple Bear — a Nairobi shop selling KIDS' SHOES (babies, toddlers, children). Each post is either ONE specific pair/style for sale, OR a non-product post.

Reply with strict minified JSON only, no prose, no code fences.

Schema:
{"is_product": true|false, "name": "<short brand + style OR generic descriptor>", "category": "<exactly one of: Sneakers, Sandals, School Shoes, Boots, Crocs, Slides, Loafers, Baby Shoes>", "reason": "<3-6 words>"}

This shop ONLY sells kids' shoes — never output a clothing/bag category. Sandals and Slides ARE valid (kids' footwear).

Rules:
- is_product = true when the caption mentions a kids' shoe and a size signal ("size 25", "EU 27", "#23", "sizes 22-27") OR a known brand/style.
...
- category: match to the EXACT list. Closed black/brown formal kids shoes → School Shoes. Rubber clogs with holes → Crocs. Slip-on slides/slippers → Slides. Infant/first-walker → Baby Shoes. Generic "shoes" with no clear type → Sneakers.
...
function coerceCategory(c) {
  ...
  if (SHOE_CATEGORIES.has(raw)) return raw;
  const lower = raw.toLowerCase();
  // Drop anything that isn't kids footwear (clothing, bags, adult-only styles).
  if (/^(cross\s*body|tote|clutch|hobo|bags?|handbags?|purse|tshirts?|tees?|shirts?|polos?|jeans?|shorts?|joggers?|tracksuits?|hoodies?|jackets?|suits?|caps?|hats?|dress(?:es)?|skirts?|trousers?|pants?|heels?|stilettos?|wedges?|pumps?)$/i.test(lower)) return null;
  if (/^(sneakers?|trainers?|canvas|rubber\s*shoes?)$/i.test(lower)) return "Sneakers";
  if (/^(sandals?|open\s*shoes?|gladiators?)$/i.test(lower)) return "Sandals";
  if (/^(school\s*shoes?|school)$/i.test(lower)) return "School Shoes";
  ...
  if (/^(loafers?|moccasins?|mary\s*janes?|doll\s*shoes?|ballet(?:\s*flats?)?|flats?|formal|dress\s*shoes?)$/i.test(lower)) return "Loafers";
  if (/^(baby\s*shoes?|first\s*walkers?|pre[\s\-]?walkers?|booties?|bootie|crib\s*shoes?|infant|newborn)$/i.test(lower)) return "Baby Shoes";
...
const SHOP_NAME = "Purple Bear";              // per-fork: shop's display name
...
export default {
  ...
  async fetch(request, env) {
    ...
    if (request.method === "GET" && path.startsWith("/p/")) {
      const SITE = "https://purplebear.essenceautomations.com";
      ...
      const desc = esc((item.description || "Quality kids' shoes in Nairobi. Tap to view and enquire on WhatsApp.").slice(0, 160));
      const html = `<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta property="og:type" content="product">
<meta property="og:site_name" content="Purple Bear">
...
<title>${title} · Purple Bear</title>
<meta http-equiv="refresh" content="0; url=${SITE}/#shop">
</head><body style="font-family:system-ui;background:#5b2a86;color:#ffffff;text-align:center;padding:40px">Opening Purple Bear…</body></html>`;
    ...
    if (request.method === "POST" && path === "/api/check-password") {
      ...
      const FALLBACK_OWNER_PASSWORD = "purplebear25";
    ...
    // Buyer → GHL proxy. NEUTRALISED on this fork: Purple Bear has no GHL form yet,
    // so we ack WITHOUT forwarding — otherwise buyers would land in another client's
    // CRM (privacy). The admin still records every buyer + sale to KV, so nothing is
    // lost. Wire a real locationId/formId here once Purple Bear has their own GHL form.
